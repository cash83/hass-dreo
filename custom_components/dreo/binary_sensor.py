...
from .pydreo.pydreoevaporativecooler import WATER_LEVEL_STATUS_KEY as EVAP_WATER_LEVEL_STATUS_KEY, WATER_LEVEL_EMPTY as EVAP_WATER_LEVEL_EMPTY
...
BINARY_SENSORS: tuple[DreoBinarySensorEntityDescription, ...] = (
    DreoBinarySensorEntityDescription(
        ...
        value_fn=lambda device: (device.water_level == WATER_LEVEL_EMPTY) if device.type == DreoDeviceType.HUMIDIFIER else ((device.water_level == EVAP_WATER_LEVEL_EMPTY) if device.type == DreoDeviceType.EVAPORATIVE_COOLER else None),
        exists_fn=lambda device: ((device.type == DreoDeviceType.HUMIDIFIER and device.is_feature_supported(WATER_LEVEL_STATUS_KEY)) or (device.type == DreoDeviceType.EVAPORATIVE_COOLER and device.is_feature_supported(EVAP_WATER_LEVEL_STATUS_KEY))),
    ),
)
...
class DreoBinarySensorHA(DreoBaseDeviceHA, BinarySensorEntity):
    ...
    def __init__(self, device: PyDreoBaseDevice, description: DreoBinarySensorEntityDescription) -> None:
